...
- **侧边栏一级面板约定**：`会议论文` 和 `日报` 两个一级面板默认收起；当前阅读页的 active 同步只更新对应日期/标签/会议轴状态，不得自动展开一级面板。点击某个一级面板标题只切换该面板自己的展开状态，不得联动修改另一个面板；点击面板内的轴切换按钮（如会议/标签、日历位置切换）后，只收起该按钮所属的一级面板。
...
- **侧边栏二级轴交互约定**：点击会议/标签/日期等二级轴按钮、轴切换按钮或日历翻页/日期按钮时，只切换数据视图并保留侧栏当前滚动位置，不得调用 `scrollPanelIntoView` 把侧栏滚回面板顶部，也不得广播 `dpr-sidebar-updated` 触发正文 markdown 预取；预取逻辑遇到 404 必须做短期负缓存，否则本地只替换 `_sidebar.md` 调试时会反复请求不存在的论文页。
