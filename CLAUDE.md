...
- **侧边栏定位约定**：`#dpr-sidebar-v2 ul/li` 的 docsify reset 会覆盖子项定位和边距；需要在同作用域内恢复 `.dpr-sidebar-paper` 的 `position/display/margin/list-style`，确保单篇论文伪元素挂在论文行且左右留白稳定。
- **侧边栏顶部入口约定**：`首页` / `使用教程` 的中文文字必须独立居中；emoji 图标绝对定位在文字左侧，不参与居中宽度计算。
- **侧边栏底部按钮约定**：dpr-sidebar-v2 大屏下只保留内部底部按钮栏，左侧为展开/收缩按钮，右侧为设置齿轮；两个按钮固定 34px 且不可被 flex 压缩，收起窄栏内必须视觉居中；不要再渲染外部 `custom-toggle-btn` 设置入口，也不要恢复无用途的刷新按钮。
- **侧边栏论文行布局约定**：英文标题固定单行；`1/2/3/4` 标记按钮 hover/focus 时在论文行右侧垂直居中显示，英文标题只预留紧凑的按钮宽度空间（当前 48px），并用两个点 `..` 作为截断提示。
