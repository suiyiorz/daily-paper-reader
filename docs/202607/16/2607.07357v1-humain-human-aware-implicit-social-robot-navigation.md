...
tldr: "社交机器人导航需感知人类行为，但显式建模成本高。HumAIN通过知识蒸馏将隐式社交线索融入规划，教师模型融合多模态数据学得鲁棒表征，学生模型轻量化部署。在轨迹预测指标上平均提升29.8%，优于SOTA。证明隐式全身线索可让资源受限平台实现类人导航意识。"
source: arxiv
...
motivation: 现有导航方法忽略隐式社交线索，难以兼顾感知准确性及实时部署需求。
method: 使用transformer教师模型融合历史图像、骨骼关键点等多模态输入，蒸馏为轻量学生模型，优化轨迹重建与特征对齐。
result: "在多个基准上轨迹预测指标平均提升29.8%，显著优于当前最优方法。"
conclusion: 隐式全身线索结合知识蒸馏，使机器人能在资源受限平台上高效、稳健地进行社交导航。
---

## 摘要
有效的社交机器人导航需要对人类行为敏感，这种敏感性常常通过微妙的骨骼线索（如步态和朝向）表现出来。我们提出了人感知隐式社交机器人导航（HumAIN），这是一种新颖的框架，通过知识蒸馏将隐式社交线索直接融合到规划循环中。我们首先采用基于Transformer的教师模型，该模型融合丰富的多模态输入，包括历史图像、骨骼关键点、机器人状态和机器人目标，以学习鲁棒的、具有人感知能力的机器人未来轨迹规划表示。为了实现实时部署，我们随后将这些知识蒸馏到一个轻量级学生模型中。通过同时优化轨迹重建和与教师的潜在特征对齐，学生模型学会从最小输入中推断复杂的社交动态。利用高效的蒸馏架构弥合预测与规划之间的差距，我们的方法使机器人能够以自适应、鲁棒且符合社交规范的方式推理人类行为。我们通过大量实验验证了HumAIN，与最先进的基线相比，轨迹预测指标平均提高了29.8%。这些结果突显了在资源受限平台上利用隐式全身线索实现类人导航感知的优势。
